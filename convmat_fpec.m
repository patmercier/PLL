...
% close all
%% PLL parameters
N = 18; % Reference clock Divide Ratio
M = 4; % DSM clock divid ratio
P = 2; % FPEC window length
...
% psd_ref_up = abs(1./(1-exp(-1j*N*wnorm_dt))).^2 * tdc_white_free/(2*pi) /N;
% This models a non-white reference noise with -20 dB/dec slope upsampled
% by N. When using it, also add a 1/(1-z^(-1)) block in the simulink model.
...
%% W_N
% This section computes the conversion matrix of multiplying piriodic
% sequences, Hmul, using (15) in the paper. These matrices are independent
% of frequency, thus placed outside the next section. 
...
%% Transfer Functions
% This section computes frequency dependent LTI conversion matrix, Hlti,
% using (8) in the paper. The conversion matrix of each noise source is
% also calculated in the same loop.

% Conversion matrix of DCO, REF and DSM.
...
Omega_vec = zeros(1,N); % Freq vector. Omega_vec(N) has no frequency shift,
% Omega_vec(1) is shifted by (N-1)*2*pi/N.
...
for index = 1:length(wnorm_dt)
    ...
    H = (I-Loop_HTF)\I;
    H_DCO(index,:) = H(N,:); % DCO noise

    H = (I-Loop_HTF)\(DCO_HTF * LPF_HTF * Kpd * W_u_ref);
    % Remove W_u_ref to see the effect of decorrelation sequence
    H_REF(index,:) = H(N,:); % REF noise

    H = (I-Loop_HTF)\(DCO_HTF * ZOH(z_vec,M) * W_u_dsm);
    % Remove W_u_dsm to see the effect of decorrelation sequence
    H_DSM(index,:) = H(N,:); % DSM quantization noise
...
% circshift by (N-k)*Npts represents shifting the frequency by
% (N-k)*2*pi/N, according to the definition of wnorm_dt.
...
% -3 because PN is defined as half of the SSB PSD.
